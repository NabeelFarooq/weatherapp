(function displayWeather() {
  ...
  async function getWeather(input) {
    ...
    try {
      ...
      if (data.cod && data.cod !== 200) {
        throw new Error(
          data.message ||
            "Location not found. Please try again with a valid location."
        );
      }
      displayWeather(data);
    } catch (err) {
  ...
  function displayWeather(data) {
    // Verify that data contains the required properties before proceeding
    if (!data || !data.weather || !data.weather[0]) {
      alert("Invalid weather data received. Please try a different location.");
      return;
    }
